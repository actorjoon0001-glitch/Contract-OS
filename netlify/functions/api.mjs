  ...
  try {
    ...
    if (id === null) {
      if (method === 'GET') {
        ...
        let query = supa
          .from(TABLE)
          .select('id, contract_no, status, stage:data->>stage, approval_at:data->signatures->approval->>signedAt, deleted_at:data->>deletedAt, id_count:data->>idCount, drawing_count:data->>drawingCount, memo:data->>memo, deposit_date:data->deposit->>date, deposit_amount:data->deposit->>amount, down_payment:data->amounts->>downPayment, permit_type:data->>permitType, owner_email:data->>ownerEmail, client_name, client_phone:data->client->>phone, site_address, showroom, salesperson, contract_date, total_amount, updated_at')
