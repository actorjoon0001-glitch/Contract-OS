...
export const Contracts = {
  list({ q = '', deleted = false } = {}) {
    // 휴지통 분리: deletedAt 값이 있으면 삭제됨(휴지통), 없으면 정상 목록
    const delCond = deleted
      ? `json_extract(data, '$.deletedAt') IS NOT NULL`
      : `json_extract(data, '$.deletedAt') IS NULL`;
    const cols = `id, contract_no, status, json_extract(data, '$.stage') AS stage, json_extract(data, '$.signatures.approval.signedAt') AS approval_at, json_extract(data, '$.deletedAt') AS deleted_at, json_extract(data, '$.idCount') AS id_count, json_extract(data, '$.drawingCount') AS drawing_count, json_extract(data, '$.memo') AS memo, json_extract(data, '$.deposit.date') AS deposit_date, json_extract(data, '$.deposit.amount') AS deposit_amount, json_extract(data, '$.amounts.downPayment') AS down_payment, json_extract(data, '$.permitType') AS permit_type, json_extract(data, '$.modelName') AS model_name, client_name, json_extract(data, '$.client.phone') AS client_phone, site_address, showroom, salesperson, contract_date, total_amount, updated_at`;
    if (q) {
      const like = `%${q}%`;
      return db
        .prepare(
          `SELECT ${cols}
           FROM contracts
           WHERE ${delCond} AND (client_name LIKE ? OR site_address LIKE ? OR contract_no LIKE ? OR showroom LIKE ? OR salesperson LIKE ?)
           ORDER BY created_at DESC, contract_no DESC`
        )
        .all(like, like, like, like, like);
    }
    return db
      .prepare(`SELECT ${cols} FROM contracts WHERE ${delCond} ORDER BY created_at DESC, contract_no DESC`)
      .all();
  },
