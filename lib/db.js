...
export const Contracts = {
  list({ q = '' } = {}) {
    if (q) {
      ...
      return db
        .prepare(
          `SELECT id, contract_no, status, client_name, site_address, contract_date, total_amount, updated_at
           FROM contracts
           WHERE client_name LIKE ? OR site_address LIKE ? OR contract_no LIKE ?
  ...
  create(data = {}) {
    ...
    const info = db
      .prepare(
        `INSERT INTO contracts (contract_no, status, client_name, site_address, contract_date, total_amount, data)
         VALUES (?, ?, ?, ?, ?, ?, ?)`
      )
      .run(contractNo, s.status, s.client_name, s.site_address, s.contract_date, s.total_amount, JSON.stringify(data));
  ...
  update(id, data = {}) {
    ...
    db.prepare(
      `UPDATE contracts
       SET status = ?, client_name = ?, site_address = ?, contract_date = ?, total_amount = ?, data = ?,
           updated_at = datetime('now','localtime')
       WHERE id = ?`
    ).run(s.status, s.client_name, s.site_address, s.contract_date, s.total_amount, JSON.stringify(data), id);
    return this.get(id);
  },
