...
export const Contracts = {
  list({ q = '', deleted = false } = {}) {
    ...
    const cols = `id, contract_no, status, json_extract(data, '$.stage') AS stage, json_extract(data, '$.signatures.approval.signedAt') AS approval_at, json_extract(data, '$.deletedAt') AS deleted_at, json_extract(data, '$.idCount') AS id_count, client_name, site_address, showroom, salesperson, contract_date, total_amount, updated_at`;
    if (q) {
      const like = `%${q}%`;
      return db
        .prepare(
