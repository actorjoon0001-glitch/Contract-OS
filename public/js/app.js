...
// ---------- 인쇄: 내용 그대로 한 페이지에 맞춤 ----------
// 계약서 전체를 측정해 A4 가로 한 장 높이에 맞게 균일 축소(비율 유지)
let printHidden = [];
function fitPrint() {
  const el = document.getElementById('contract');
  if (!el) return;
  const PRINT_W = 1060; // A4 가로 인쇄 가능 폭(px, 8mm 여백)
  const PRINT_H = 731;  // A4 가로 인쇄 가능 높이(px)
  ...
  // 화면 전용 요소(안내 배너 등)는 인쇄에서 빠지므로 측정에서도 제외
  printHidden = [...el.querySelectorAll('.no-print')];
  printHidden.forEach((n) => { n.dataset.prevDisp = n.style.display; n.style.display = 'none'; });
  // 인쇄 폭 기준으로 메모 textarea 높이 재계산
  el.querySelectorAll('textarea.f').forEach(autoGrow);
  // 측정 후 한 페이지에 맞게 축소
  const h = el.scrollHeight;
  const scale = Math.min(1, PRINT_H / h);
...
function unfitPrint() {
  ...
  printHidden.forEach((n) => { n.style.display = n.dataset.prevDisp || ''; delete n.dataset.prevDisp; });
  printHidden = [];
}
